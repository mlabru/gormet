...
# log file
LOGF="logs/gormet.$HOST.$TDATE.log"

# logger
echo "Início de processamento: " $(date '+%Y-%m-%d %H:%M') > $LOGF

# ckeck if another instance of loader is running
DI_PID_GORMET=`ps ax | grep -w python3 | grep -w gormet.py | awk '{ print $1 }'`

if [ ! -z "$DI_PID_GORMET" ]; then
    # log warning
    echo "[`date`]: process gormet is already running. Waiting..." >> $LOGF
    # kill process
